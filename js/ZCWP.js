...
ZCWP.general = {
  ...
  modify_chart:function (element,key,category,subCategory) {
    var value = ''
    switch (element.type) {
      case("checkbox") :
   ...
   */
  set_bg_type :  function (element,category,subCategory) {
    var bgType = element.options[element.selectedIndex].value;
    document.getElementById('backgroundColor1'+category+subCategory).dataset.bgtype = bgType;
    if (bgType == "gradiant") {
      var domHolder = document.getElementById("backgroundColor2"+category+subCategory);
    ...
    this.set_bg_color(category,subCategory);
  },
  set_bg_color : function (category,subCategory) {
    var dataObj = {};
    dataObj[category] = {};
    var bgType = document.getElementById('backgroundColor1'+category+subCategory).dataset.bgtype;
    if (category != subCategory) {
      dataObj[category][subCategory] = {};
      if (bgType == "gradiant") {
        //Set background-color-1 attr
      dataObj[category][subCategory]['backgroundColor1'] = document.getElementById('backgroundColor1'+category+subCategory).value;
      dataObj[category][subCategory]['backgroundColor2'] = document.getElementById('backgroundColor2'+category+subCategory).value;
      update_zingchart("modify", dataObj);
      } else {
      //Set background-color-1 attr
        dataObj[category][subCategory]['backgroundColor1'] = document.getElementById('backgroundColor1'+category+subCategory).value;
        dataObj[category][subCategory]['backgroundColor2'] = document.getElementById('backgroundColor1'+category+subCategory).value;
        update_zingchart("modify", dataObj);
      }
    } else {
      if (bgType == "gradiant") {
        //Set background-color-1 attr
        dataObj[category]['backgroundColor1'] = document.getElementById('backgroundColor1'+category+subCategory).value;
        dataObj[category]['backgroundColor2'] = document.getElementById('backgroundColor2'+category+subCategory).value;
        update_zingchart("modify", dataObj);
      } else {
      //Set background-color-1 attr
        dataObj[category]['backgroundColor1'] = document.getElementById('backgroundColor1'+category+subCategory).value;
        dataObj[category]['backgroundColor2'] = document.getElementById('backgroundColor1'+category+subCategory).value;
        update_zingchart("modify", dataObj);
      }
    }
  },
   ...
   */
  set_line : function (category,subCategory) {
    var dataObj = {};
    dataObj[category] = {};
    if (category != subCategory) {
      dataObj[category][subCategory] ={};
...
ZCWP.label  = {
  new_label: function (NoJSON) {
    ...
    labelConfigId++;
    clnlLbl.dataset.count = labelConfigId;
    ...
    //NoJSON argument is oly for making sure that
    // when chart reloads its data it won't creat extera labels
    if (arguments.length < 1) {
      if (labelConfigId == 1 ) {
        var chartData = zingchart.exec(chartID, 'getdata');
        if( typeof chartData['graphset'][0]['labels'] == "undefined") {
          var dataObj = {
            labels :[]
          };
          var vals ={}
          ...
          var vals ={};
          chartData['graphset'][0]['labels'].push(vals);
          update_zingchart("modify", chartData['graphset'][0]);
        }
      } else {
        var chartData = zingchart.exec(chartID, 'getdata');
        var empty ={}
        chartData['graphset'][0]['labels'].push(empty);
        update_zingchart("modify", chartData['graphset'][0]);
      }
    }
  },
 ...
 * Label Modify chart
 */
  modify_chart_label : function (id,type,key,category,subcategory,count) {
    var value = ''
    switch (type) {
      case("checkbox") :
    ...
    // Get chart JSON
    var chartData = zingchart.exec(chartID, 'getdata');
    if (typeof chartData['graphset'][0]['labels']  == "undefined" ){ //Empty array situation, createing a new label
      var dataObj = {
        labels :[],
      };
      var vals = {};
      ...
      if (count == chartData['graphset'][0]['labels'].length) {//New elemnt case we hace to push it 
        if (key == 'callout-hook') {
          calloutHook =[];
          calloutHook.push(document.getElementById("callout-hooklabelsX").value);
          calloutHook.push(document.getElementById("callout-hooklabelsY").value);
          chartData['graphset'][0]['labels'].push(calloutHook);
        } else {
          var vals = {};
          vals[key] = value;
          chartData['graphset'][0]['labels'].push(vals);
        ...
        chartData['graphset'][0]['labels'][count][key] = value;
        update_zingchart("modify", chartData['graphset'][0]);
      }
    }
  },
   ...
   */ 
  set_bg_type_label : function (id,category,subCategory,count) {
  count = (count != 0) ? count : ''; 
  var type = document.getElementById(id);
  var bgTypelbl = type.options[type.selectedIndex].value;
  document.getElementById("backgroundColor1"+subCategory+category+count).dataset.bgtype = bgTypelbl;
  if (bgTypelbl =="gradiant") {
  ...
  this.set_bg_color_label(category,subCategory,count);
  },
  set_bg_color_label : function (category,subCategory,count) {
  count = (count != 0) ? count : ''; 
  bgTypelbl = document.getElementById("backgroundColor1"+subCategory+category+count).dataset.bgtype ;
  // Get chart JSON
  var chartData = zingchart.exec(chartID, 'getdata');
  if (typeof chartData['graphset'][0]['labels'] == "undefined" ){ //Empty array situation, createing a new label
    var dataObj = {
    ...
    if (count == chartData['graphset'][0]['labels'].length) {
      if (bgTypelbl == "gradiant") {
        //Set background-color-1 attr
        var vals = {};
      ...
      if (bgTypelbl == "gradiant") {
        ...
        chartData['graphset'][0]['labels'][(count == "" ) ? 0 : count]['background-color-2'] = document.getElementById('backgroundColor2'+subCategory+category+count).value;
      } else {
        //Set background-color-1 attr
        chartData['graphset'][0]['labels'][(count == "" ) ? 0 : count]['background-color-1'] = document.getElementById('backgroundColor1'+subCategory+category+count).value;
...
function modify_chart_series(element,type,key,category,subCategory) {
  ...
  // Get chart JSON
  var chartData = zingchart.exec(chartID, 'getdata');
  var chartSeries = chartData['graphset'][0]['series']; // Ternary operator to check to see if 'labels' exists
      ...
      for(attr in elementsJSON ) {
        ...
        if (attr == "background-color-1") {
          ...
          if (elementsJSON[attr] != elementsJSON["background-color-2"]) {
            ...
            document.getElementById("backgroundColor1"+bgEndId).dataset.bgtype = "gradiant"; 
...
function load_empty_labels() {
  if(document.getElementById('zingcharts-javaScript').value.trim() != ""){
    ...
    for(var i = 0 ,len= chartJSON['graphset'][0]["labels"].length-1 ; i<len;i++ ) {
      ZCWP.label.new_label(true);
    }
  }
}

function load_label_data(element) {
  if(document.getElementById('zingcharts-javaScript').value.trim() != ""){
    var chartJSON = JSON.parse(document.getElementById('zingcharts-javaScript').value);
    var labelInfo = chartJSON['graphset'][0]["labels"][element.nextElementSibling.dataset.count];
    var elements  = element.nextElementSibling.childNodes;
    var counter   = (element.nextElementSibling.dataset.count !="0") ? element.nextElementSibling.dataset.count : "";
    if (labelInfo["background-color-1"]) {
      if (labelInfo["background-color-1"] == labelInfo["background-color-2"]) {
        document.getElementById("backgroundColor1labelslabels"+ counter).value = labelInfo["background-color-1"];
        document.getElementById("backgroundColor2labelslabels"+ counter).value = labelInfo["background-color-1"];
      } else {
        document.getElementById("backgroundTypelabelslabels"   + counter).selectedIndex = 1;
        document.getElementById("backgroundColor1labelslabels" + counter).value = labelInfo["background-color-1"];
        document.getElementById("backgroundColor2labelslabels" + counter).value = labelInfo["background-color-2"];
        document.getElementById("backgroundColor2labelslabels" + counter).style.visibility = "visible";
      } 
    };
    for(el in elements) {
      if (elements[el].nodeName != "#text") {
        try {
          if (typeof labelInfo[elements[el].dataset.key] != "undefined") {
            switch (elements[el].type) {
            case ("checkbox"):
              elements[el].checked = labelInfo[elements[el].dataset.key];
              break;
            case("select-one") :
              elements[el].value  = labelInfo[elements[el].dataset.key];
              break;
            default:
              elements[el].value = labelInfo[elements[el].dataset.key];
            }
...
function update_zingchart(method,JSON) {
  zingchart.exec(chartID,method, {
    data : JSON
  });
    create_json();
}
...
/*         Under Dev
/*******************************/
//Test case exports goes here
/*
var exports = module.exports = {};
exports.modify() = ZCWP.general.modify_chart(); 
*/
  ...
  /* No longer needed
  set_border : function (category,subCategory) {
    var dataObj = {};
    dataObj[category] = {};
    if (category != subCategory) {
      dataObj[category][subCategory] ={};
